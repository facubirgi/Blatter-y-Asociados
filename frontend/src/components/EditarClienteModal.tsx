...
import { type Cliente, type CondicionFiscal } from '../services/clienteService';

interface EditarClienteModalProps {
  isOpen: boolean;
  cliente: Cliente | null;
  onClose: () => void;
  onSubmit: (id: string, data: ClienteUpdateData) => Promise<void>;
}
...
export default function EditarClienteModal({ isOpen, cliente, onClose, onSubmit }: EditarClienteModalProps) {
  const [formData, setFormData] = useState<ClienteUpdateData>({
    nombre: '',
    cuit: '',
    fechaAlta: '',
    contacto: '',
  ...
  useEffect(() => {
    if (cliente) {
      setFormData({
        ...
        cuit: cliente.cuit,
        fechaAlta: cliente.fechaAlta.split('T')[0], // Convertir a formato YYYY-MM-DD
        contacto: cliente.contacto,
        esClienteFijo: cliente.esClienteFijo,
        montoMensualidad: cliente.montoMensualidad,
